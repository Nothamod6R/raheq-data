 ...
 *
 * Returns the normalized Mushaf layout for a single page (page -> line -> word).
 * Reads from database/quran/text/layout/normalized/page-NNN.json (generated by
 * scripts/normalize-mushaf-layout.js).
 *
 * Additive endpoint — existing /api/quran/* consumers are unchanged.
 */
export const getQuranLayoutPage = async (request, reply) => {
    const rawPage = Number(request.params.page);
    if (!Number.isInteger(rawPage) || rawPage < 1 || rawPage > 604) {
        return reply.status(404).send({
            error: 'Not Found',
            message: 'Invalid page. Must be an integer between 1 and 604.',
        });
    }

    const cacheKey = `quran_layout:page:${rawPage}`;
    const data = await handleCache(cacheKey, async () => {
        const fileName = `page-${String(rawPage).padStart(3, '0')}.json`;
        const filePath = path.join(process.cwd(), 'database', 'quran', 'text', 'layout', 'normalized', fileName);
