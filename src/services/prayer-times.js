...
/* are supported:
   - muslim_world_league:     18° / 17°        (MWL, 2007 consensus)
   - north_america:           15° / 15°        (ISNA)
   - egyptian:                19.5° / 17.5°    (Egyptian General Authority)
   - umm_al_qura:             18.5° / 90 min after Maghrib
                                (120 min during Ramadan, Umm al-Qura, Makkah)
   - karachi:                 18° / 18°        (University of Islamic Sciences)
   - gulf_region:             19.5° / 90 min   (Dubai/UAE method)
   - kuwait:                  18° / 17.5°
   - qatar:                   18° / 90 min
   - singapore:               20° / 18°        (MUIS)
   - france:                  12° / 12°        (Union des Organisations Islamiques)
   - turkey:                  18° / 17°        (Diyanet)
   - russia:                  16° / 15°        (Smart Habib/Mohamad Said)
   - moonsighting_committee:  18° / 18°
   - dubai:                   18.2° / 18.2°
   - jakim:                   20° / 18°        (Malaysia, JAKIM)
   - tunisia:                 18° / 18°
   - algeria:                 18° / 17°
   - indonesia:               20° / 18°        (KEMENAG)
   - morocco:                 19° / 17°        (Ministère des Habous)
*/
export const CALCULATION_METHODS = {
    muslim_world_league: { fajrAngle: 18, ishaAngle: 17 },
    north_america: { fajrAngle: 15, ishaAngle: 15 },
    egyptian: { fajrAngle: 19.5, ishaAngle: 17.5 },
    umm_al_qura: { fajrAngle: 18.5, ishaMinutes: 90, ramadanIshaMinutes: 120 },
    karachi: { fajrAngle: 18, ishaAngle: 18 },
    gulf_region: { fajrAngle: 19.5, ishaMinutes: 90 },
    kuwait: { fajrAngle: 18, ishaAngle: 17.5 },
    qatar: { fajrAngle: 18, ishaMinutes: 90 },
    ...
    moonsighting_committee: { fajrAngle: 18, ishaAngle: 18 },
...
export const SUPPORTED_METHODS = Object.keys(CALCULATION_METHODS);

export const SUPPORTED_MADHABS = ['shafi', 'hanafi'];

const MADHAB_ASR_FACTOR = { shafi: 1, hanafi: 2 };
...
const sunPosition = (jd) => {
    ...
    const declination = asind(sind(obliquity) * sind(eclipticLongitude));
    let rightAscension = atan2d(
        cosd(obliquity) * sind(eclipticLongitude),
        cosd(eclipticLongitude)
    );
    if (rightAscension < 0) rightAscension += 360;

    const equationOfTime = meanLongitude / 15 - rightAscension / 15;
    return { declination, equationOfTime };
};

const sunHourAngle = (altitude, latitude, declination) => {
    const cosH =
        (sind(altitude) - sind(latitude) * sind(declination)) /
...
const solarNoon = (longitude, utcOffset, equationOfTime) =>
    12 * 60 + utcOffset * 60 - longitude * 4 - equationOfTime * 60;

const asrAltitude = (latitude, declination, factor) => {
    const tangent = Math.abs(tand(latitude - declination));
    return Math.atan(1 / (tangent + factor)) * (180 / Math.PI);
};

const highLatCorrection = (prayer, angle, sunrise, maghrib) => {
    let night = sunrise - maghrib;
    if (night <= 0) night += 24 * 60;
    if (!Number.isFinite(night) || night <= 0) return null;

    const portion = angle / 60;
    if (prayer === 'fajr') return sunrise - portion * night;
    return maghrib + portion * night;
};

export const computePrayerTimes = (options) => {
    const {
        year,
        month,
        day,
    ...
    const asrFactor = MADHAB_ASR_FACTOR[madhab];

    const jd = julianDate({ year, month, day });
    const { declination, equationOfTime } = sunPosition(jd);

    const dhuhr = solarNoon(longitude, utcOffset, equationOfTime);

    const hourAngle = (altitude) => sunHourAngle(altitude, latitude, declination);
    const minutes = (h) => (h === null ? null : h * 4);

    const riseSetMinutes = minutes(hourAngle(-SUNRISE_SUNSET_ANGLE));
    const sunrise = riseSetMinutes === null ? null : dhuhr - riseSetMinutes;
    const maghrib = riseSetMinutes === null ? null : dhuhr + riseSetMinutes;

    let fajr = minutes(hourAngle(-methodParams.fajrAngle));
    fajr = fajr === null ? null : dhuhr - fajr;

    let isha;
    if (typeof methodParams.ishaMinutes === 'number') {
        let interval = methodParams.ishaMinutes;
        if (methodParams.ramadanIshaMinutes && isRamadan({ year, month, day })) {
            interval = methodParams.ramadanIshaMinutes;
        }
        isha = maghrib === null ? null : maghrib + interval;
    } else {
        let t = minutes(hourAngle(-methodParams.ishaAngle));
        isha = t === null ? null : dhuhr + t;
    }

    if (fajr === null && sunrise !== null && maghrib !== null) {
        fajr = highLatCorrection('fajr', methodParams.fajrAngle, sunrise, maghrib);
    }
    if (isha === null && sunrise !== null && maghrib !== null) {
        const ishaAngle = methodParams.ishaAngle ?? 18;
        isha = highLatCorrection('isha', ishaAngle, sunrise, maghrib);
    }

    const altitude = asrAltitude(latitude, declination, asrFactor);
    let asr = minutes(hourAngle(altitude));
    if (asr !== null) asr = dhuhr + asr;

    return {
        fajr: fajr === null ? null : Math.round(fajr),
        sunrise: sunrise === null ? null : Math.round(sunrise),
        dhuhr: Math.round(dhuhr),
        asr: asr === null ? null : Math.round(asr),
        maghrib: maghrib === null ? null : Math.round(maghrib),
        isha: isha === null ? null : Math.round(isha)
    };
};

// ------------------------- Umm al-Qura / Ramadan helper ---------------------
// Official Umm al-Qura Islamic calendar (Umm al-Qura University, Makkah),
// used to resolve Umm Al-Qura's 120-minute Isha interval during Ramadan. The
// civil Umm al-Qura date is looked up from the official lunation table below
// (Modified Chronological Julian Day Numbers, MCJDN = CJDN - 2400000) rather
// than an arithmetic tabular approximation, so the Ramadan check cannot drift
...
const ummAlQuraHijriMonth = ({ year, month, day }) => {
    let m = month;
    let y = year;
    if (m < 3) {
        y -= 1;
    ...
    const first = UMM_AL_QURA_MONTH_STARTS[0];
    const last = UMM_AL_QURA_MONTH_STARTS[UMM_AL_QURA_MONTH_STARTS.length - 1];
    if (mcjdn < first || mcjdn >= last) return null;

    let i = 0;
    while (UMM_AL_QURA_MONTH_STARTS[i] <= mcjdn) i += 1;
    const iln = i + 16260;
